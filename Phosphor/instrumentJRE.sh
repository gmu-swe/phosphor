...
if [ -z "$JAVA_HOME" ]; then
	...
	if [ ! -d "target/jre-inst-int" ]; then
		...
		java -Xmx6g -jar target/Phosphor-0.0.3-SNAPSHOT.jar -serialization -forceUnboxAcmpEq -withEnumsByValue $INST_HOME target/jre-inst-int;
	...
	if [ ! -d "target/jre-inst-obj" ]; then
		...
		java -Xmx6g -jar target/Phosphor-0.0.3-SNAPSHOT.jar -serialization -multiTaint -forceUnboxAcmpEq -withEnumsByValue $INST_HOME target/jre-inst-obj;
