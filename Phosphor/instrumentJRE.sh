...
if [ -z "$JAVA_HOME" ]; then
	...
	if [ ! -d "target/jre-inst-int" ]; then
		...
		java -Xmx6g -XX:MaxPermSize=512m -jar target/Phosphor-0.0.1-SNAPSHOT.jar -forceUnboxAcmpEq -withEnumsByValue $JAVA_HOME target/jre-inst-int;
	...
	if [ ! -d "target/jre-inst-obj" ]; then
		...
		java -Xmx6g -XX:MaxPermSize=512m  -jar target/Phosphor-0.0.1-SNAPSHOT.jar -multiTaint -forceUnboxAcmpEq -withEnumsByValue $JAVA_HOME target/jre-inst-obj;
	...
	if [ ! -d "target/jre-inst-implicit" ]; then
		...
		java -Xmx6g -XX:MaxPermSize=512m -jar target/Phosphor-0.0.1-SNAPSHOT.jar -controlTrack -multiTaint -forceUnboxAcmpEq -withEnumsByValue $JAVA_HOME target/jre-inst-implicit;
