...
if [ -z "$JAVA_HOME" ]; then
	...
	if [ ! -d "target/jre-inst-implicit" ]; then
		...
		java -Xmx6g -jar target/Phosphor-0.0.4-SNAPSHOT.jar -controlTrack -controlTrackExceptions -multiTaint -forceUnboxAcmpEq -withEnumsByValue $INST_HOME target/jre-inst-implicit;
	else
