...
if [ -z "$JAVA_HOME" ]; then
	...
	if [ ! -d "target/jre-inst-int" ]; then
		...
		java -Xmx6g -Dphosphor.verify=true -jar target/Phosphor-0.0.5-SNAPSHOT.jar -serialization -forceUnboxAcmpEq -withEnumsByValue $INST_HOME target/jre-inst-int;
	else
	...
	if [ ! -d "target/jre-inst-implicit" ]; then
		...
		java -Xmx6g -Dphosphor.verify=true -jar target/Phosphor-0.0.5-SNAPSHOT.jar -controlTrack -multiTaint -forceUnboxAcmpEq -withEnumsByValue -summarizeMethodsNotCalled $INST_HOME target/jre-inst-implicit;
	else
