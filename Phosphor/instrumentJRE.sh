...
if [ -z "$JAVA_HOME" ]; then
	...
	if [ ! -d "target/jre-inst-obj" ]; then
		...
		java -Xmx6g -Dphosphor.verify=true -jar target/Phosphor-0.0.5-SNAPSHOT.jar -serialization -multiTaint -forceUnboxAcmpEq -withEnumsByValue $INST_HOME target/jre-inst-obj;
	else
	...
	if [ ! -d "target/jre-inst-implicit" ]; then
		...
		java -Xmx6g -Dphosphor.verify=true -jar target/Phosphor-0.0.5-SNAPSHOT.jar -controlTrack -multiTaint -forceUnboxAcmpEq -withEnumsByValue $INST_HOME target/jre-inst-implicit;
	else
	...
	if [ ! -d "target/jre-inst-binding-control" ]; then
		...
		java -Xmx6g -Dphosphor.verify=true -jar target/Phosphor-0.0.5-SNAPSHOT.jar -controlTrack -multiTaint -bindingControlTracking -forceUnboxAcmpEq -withEnumsByValue $INST_HOME target/jre-inst-binding-control;
	else
