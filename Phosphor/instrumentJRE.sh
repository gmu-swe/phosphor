...
if [ -z "$JAVA_HOME" ]; then
	...
	if [ ! -d "target/jre-inst-int" ]; then
		...
		java -Xmx6g -Dphosphor.verify=true -jar target/Phosphor-0.0.4.jar -serialization -forceUnboxAcmpEq -withEnumsByValue $INST_HOME target/jre-inst-int;
	else
		echo "Not regenerating int tag instrumented JRE\n";
	fi
	if [ ! -d "target/jre-inst-obj" ]; then
			echo "Creating obj tag instrumented JRE\n";
		java -Xmx6g -Dphosphor.verify=true -jar target/Phosphor-0.0.4.jar -serialization -multiTaint -forceUnboxAcmpEq -withEnumsByValue $INST_HOME target/jre-inst-obj;
	else
	...
	if [ ! -d "target/jre-inst-implicit" ]; then
		...
		java -Xmx6g -Dphosphor.verify=true -jar target/Phosphor-0.0.4.jar -controlTrack -multiTaint -forceUnboxAcmpEq -withEnumsByValue $INST_HOME target/jre-inst-implicit;
	else
