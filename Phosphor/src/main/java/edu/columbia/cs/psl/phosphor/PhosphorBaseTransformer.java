...
public abstract class PhosphorBaseTransformer {
    ...
    public byte[] signalAndTransform(ClassLoader loader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain,
                                     byte[] classFileBuffer, boolean isAnonymousClassDefinition) throws IllegalClassFormatException {
        try {
            synchronized(PhosphorBaseTransformer.class) {
                isBusyTransforming++;
            }
            return transform(loader, className, classBeingRedefined, protectionDomain, classFileBuffer, isAnonymousClassDefinition);
        } finally {
            synchronized(PhosphorBaseTransformer.class) {
                isBusyTransforming--;
            }
