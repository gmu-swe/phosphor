...
public class ArrayReflectionMasker {
	...
	public static void set$$PHOSPHORTAGGED(Object obj, int idxtaint, int idx, Object val, ControlTaintTagStack ctrl) {
		if (obj != null && !obj.getClass().isArray()) {
			//in this case obj will be boxed, and we need to pull the taint out of val when we unbox it
			if (obj instanceof MultiDTaintedBooleanArrayWithIntTag)
				setBooleanInt$$PHOSPHORTAGGED(obj, idxtaint, idx, tryToGetTaint(val), (Boolean) val, ctrl);
			else if (obj instanceof MultiDTaintedByteArrayWithIntTag)
				setByteInt$$PHOSPHORTAGGED(obj, idxtaint, idx, tryToGetTaint(val), (Byte) val, ctrl);
			else if (obj instanceof MultiDTaintedCharArrayWithIntTag)
				setCharInt$$PHOSPHORTAGGED(obj, idxtaint, idx, tryToGetTaint(val), (Character) val, ctrl);
			else if (obj instanceof MultiDTaintedDoubleArrayWithIntTag)
				setDoubleInt$$PHOSPHORTAGGED(obj, idxtaint, idx, tryToGetTaint(val), (Double) val, ctrl);
			else if (obj instanceof MultiDTaintedFloatArrayWithIntTag)
				setFloatInt$$PHOSPHORTAGGED(obj, idxtaint, idx, tryToGetTaint(val), (Float) val, ctrl);
			else if (obj instanceof MultiDTaintedIntArrayWithIntTag)
				setIntInt$$PHOSPHORTAGGED(obj, idxtaint, idx, tryToGetTaint(val), (Integer) val, ctrl);
			else if (obj instanceof MultiDTaintedLongArrayWithIntTag)
				setLongInt$$PHOSPHORTAGGED(obj, idxtaint, idx, tryToGetTaint(val), (Long) val, ctrl);
			else if (obj instanceof MultiDTaintedShortArrayWithIntTag)
				setShortInt$$PHOSPHORTAGGED(obj, idxtaint, idx, tryToGetTaint(val), (Short) val, ctrl);
			else
				throw new ArrayStoreException("Got passed an obj of type " + obj + " to store to");
		} else
			Array.set(obj, idx, val);
	...
	public static void setInt$$PHOSPHORTAGGED(Object obj, int idxtaint, int idx, Object val, ControlTaintTagStack ctrl) {
		if (obj != null && !obj.getClass().isArray()) {
			//in this case obj will be boxed, and we need to pull the taint out of val when we unbox it
			if (obj instanceof MultiDTaintedBooleanArrayWithIntTag)
				setBooleanInt$$PHOSPHORTAGGED(obj, idxtaint, idx, tryToGetTaint(val), (Boolean) val, ctrl);
	...
	public static void setBooleanInt$$PHOSPHORTAGGED(Object obj, int idxtaint, int idx, int taint, boolean val, ControlTaintTagStack ctrl) {
		taint = SimpleMultiTaintHandler.combineTags(taint, ctrl);
		setBoolean$$PHOSPHORTAGGED(obj, idxtaint, idx, taint, val);
	}

	public static void setByteInt$$PHOSPHORTAGGED(Object obj, int idxtaint, int idx, int taint, byte val, ControlTaintTagStack ctrl) {
		taint = SimpleMultiTaintHandler.combineTags(taint, ctrl);
		setByte$$PHOSPHORTAGGED(obj, idxtaint, idx, taint, val);
	}

	public static void setCharInt$$PHOSPHORTAGGED(Object obj, int idxtaint, int idx, int taint, char val, ControlTaintTagStack ctrl) {
		taint = SimpleMultiTaintHandler.combineTags(taint, ctrl);
		setChar$$PHOSPHORTAGGED(obj, idxtaint, idx, taint, val);
	}

	public static void setDoubleInt$$PHOSPHORTAGGED(Object obj, int idxtaint, int idx, int taint, double val, ControlTaintTagStack ctrl) {
		taint = SimpleMultiTaintHandler.combineTags(taint, ctrl);
		setDouble$$PHOSPHORTAGGED(obj, idxtaint, idx, taint, val);
	}

	public static void setFloatInt$$PHOSPHORTAGGED(Object obj, int idxtaint, int idx, int taint, float val, ControlTaintTagStack ctrl) {
		taint = SimpleMultiTaintHandler.combineTags(taint, ctrl);
		setFloat$$PHOSPHORTAGGED(obj, idxtaint, idx, taint, val);
	}

	public static void setIntInt$$PHOSPHORTAGGED(Object obj, int idxtaint, int idx, int taint, int val, ControlTaintTagStack ctrl) {
		taint = SimpleMultiTaintHandler.combineTags(taint, ctrl);
		setInt$$PHOSPHORTAGGED(obj, idxtaint, idx, taint, val);
	}

	public static void setLongInt$$PHOSPHORTAGGED(Object obj, int idxtaint, int idx, int taint, long val, ControlTaintTagStack ctrl) {
		taint = SimpleMultiTaintHandler.combineTags(taint, ctrl);
		setLong$$PHOSPHORTAGGED(obj, idxtaint, idx, taint, val);
	}

	public static void setShortInt$$PHOSPHORTAGGED(Object obj, int idxtaint, int idx, int taint, short val, ControlTaintTagStack ctrl) {
		taint = SimpleMultiTaintHandler.combineTags(taint, ctrl);
		setShort$$PHOSPHORTAGGED(obj, idxtaint, idx, taint, val);
	}
