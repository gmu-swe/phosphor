		...
		for (final Class c : m.getParameterTypes()) {
			if (c.isArray()) {
				if (c.getComponentType().isPrimitive()) {
					//1d primitive array
					madeChange = true;
					newArgs.add(TaintUtils.TAINT_TAG_ARRAY_CLASS);
					newArgs.add(c);
					continue;
				} else {
				...
				madeChange = true;
				newArgs.add(TaintUtils.TAINT_TAG_CLASS);
				newArgs.add(c);
				continue;
			} else {
				//anything else
				...
				if (t.getSort() == Type.ARRAY) {
					if (t.getElementType().getSort() != Type.OBJECT) {
						if (t.getDimensions() == 1) {
							newParams.add(TaintUtils.TAINT_TAG_ARRAY_CLASS);
						} else {
					...
					newParams.add(TaintUtils.TAINT_TAG_CLASS);
				}
				newParams.add(c);
			}
			if(Configuration.IMPLICIT_TRACKING)
...
//				System.out.println("FAA: " +m + " "+owner + " " + Arrays.toString(in));
		...
		} else if ((in == null && m != null && (m.getParameterTypes().length == 1 || (m.getParameterTypes().length == 2))) || (in != null && j != in.length - 1)) {
			if (in == null) {
				ret.a = new Object[2];
				ret.a[0] = ctrl;
				j++;
