...
public class BranchNotTakenWrapperGenerator {
	...
	public void generate(ClassVisitor cv, String owner, MethodNode originalMethod) {
...
//		mv.visitLabel(start);
		try {
			for (ForceControlStoreAdvice advice : analyze(owner, originalMethod)) {
				advice.apply(mv, lvm, analyzer, owner);
			}
