...
public class MethodArgReindexer extends MethodVisitor {
	...
	public MethodArgReindexer(MethodVisitor mv, int access, String name,
			String desc, String originalDesc, MethodNode lvStore) {
		...
		this.firstFrameLocals = new Type[origNumArgs];
		int ffl = 0;
		if (!isStatic) {
		...
		this.oldArgMappings = new int[originalLastArgIdx + 1];
		...
		if (TaintUtils.DEBUG_FRAMES || TaintUtils.DEBUG_LOCAL)
			System.out.println(name + " origLastArg " + originalLastArgIdx
					+ ", oldvarcount = " + oldVarCount
	...
	public void visitLocalVariable(String name, String desc, String signature,
		...
		if (index < originalLastArgIdx) {
			...
			if (!found)
				lvStore.localVariables.add(
						new LocalVariableNode(name, desc, signature, null, null, index));
		}
		...
		if (!isStatic && index == 0)
			...
			if (shadow != null)
				super.visitLocalVariable(name + "_TAINT",
						shadow, null, start, end, oldArgMappings[index] - 1);
			super.visitLocalVariable(name, desc, signature, start, end, oldArgMappings[index]);
			
			if (index == originalLastArgIdx - 1
					&& Configuration.IMPLICIT_TRACKING) {
				super.visitLocalVariable("PhopshorImplicitTaintTrackingFromParent",
						Type.getDescriptor(ControlTaintTagStack.class),
						null, start, end, oldArgMappings[index] + 1);
			}
			...
			if (index == originalLastArgIdx - 1
				...
				super.visitLocalVariable("TAINT_STUFF_TO_IGNORE_HAHA",
						"Ljava/lang/Object;",
						null, start, end,
						oldArgMappings[index] + (Configuration.IMPLICIT_TRACKING ? 2 : 1));
			}

			if ((index == originalLastArgIdx - Type.getType(desc).getSize())
					&& hasPreAllocedReturnAddr) {
				super.visitLocalVariable("PHOSPHORPREALLOCRETURNHAHA",
						newReturnType.getDescriptor(),
						null, start, end,
						oldArgMappings[index] + (Configuration.IMPLICIT_TRACKING ? 2 : 1));
			}
		} else {
	...
	public void visitFrame(int type, int nLocal, Object[] local, int nStack, Object[] stack) {
		...
		if (TaintUtils.DEBUG_FRAMES) {
			...
			System.out.println("Pre-reindex Frame: "
					+ Arrays.toString(local) + ";" + nLocal + " ; " + type);
		}
		...
		if (type == Opcodes.F_FULL || type == Opcodes.F_NEW) {
			...
			if (origNumArgs == 0 && hasPreAllocedReturnAddr) {
				// System.out.println("Adding local storage for "
				// + newReturnType.getInternalName() + " At " + newIdx);
			...
			for (int i = 0; i < origNLocal; i++) {
				...
				if (i < numLocalsToIterateOverForArgs) {
					...
					if (lastWasTop2Words) {
						...
						if (t.getSort() == Type.ARRAY
								...
								&& t.getDimensions() == 1) {
							remappedLocals[newIdx] =
								TaintUtils.getShadowTaintType(t.getDescriptor());
							
							if (TaintUtils.DEBUG_FRAMES)
								System.out.println("Adding taint storage for local type "
										+ local[i]);
							newIdx++;
							nLocal++;
						...
						if (!(local[i] != Opcodes.TOP
									&& local[i] instanceof String
									&& ((String) local[i]).charAt(1) == '[')) {
							if (local[i] != Opcodes.TOP) {
								try {
									Type argType = TaintAdapter.getTypeForStackType(local[i]);
									remappedLocals[newIdx] =
										TaintUtils.getShadowTaintTypeForFrame(argType.getDescriptor());
									newIdx++;
									nLocal++;
									// System.out.println("Adding taint storage for local type "
									// + local[i] +", ffl is " + firstFrameLocals[i]);
								} catch (IllegalArgumentException ex) {
									System.err.println("Locals were: "
											+ Arrays.toString(local) + ", curious about " + i);
									throw ex;
								}
							}
						}
				...
				if (local[i] != Opcodes.TOP
						&& local[i] instanceof String
						&& ((String) local[i]).length() > 1
						&& ((String) local[i]).charAt(1) == '['
						&& Type.getObjectType((String) local[i])
								.getElementType().getSort() != Type.OBJECT) {
					remappedLocals[newIdx] = MultiDTaintedArray.getTypeForType(
							Type.getObjectType((String) local[i])).getInternalName();
				}
				newIdx++;
		...
		// + name + desc + ":\t\t" + Arrays.toString(remappedLocals));
		ArrayList<Object> newStack = new ArrayList<Object>();
		int origNStack = nStack;
		for (int i = 0; i < origNStack; i++) {
			if (stack[i] == Opcodes.INTEGER
					|| stack[i] == Opcodes.FLOAT
					|| stack[i] == Opcodes.LONG
					|| stack[i] == Opcodes.DOUBLE) {
				if (TaintUtils.DEBUG_FRAMES)
			...
			if (stack[i] != Opcodes.TOP
					&& stack[i] instanceof String
					&& ((String) stack[i]).charAt(1) == '['
					&& Type.getObjectType((String) stack[i])
							.getElementType().getSort() != Type.OBJECT) {
				newStack.add(MultiDTaintedArray.getTypeForType(
							Type.getObjectType((String) stack[i])).getInternalName());
			} else
		...
		if (TaintUtils.DEBUG_FRAMES)
			System.out.println("Post-adjust Frame: "
					+ Arrays.toString(remappedLocals) + ";" + Arrays.toString(stack2));
		super.visitFrame(type, nLocal, remappedLocals, nStack, stack2);
		if (TaintUtils.DEBUG_FRAMES)
			System.out.println("Post-visit Frame: "
					+ Arrays.toString(remappedLocals) + ";" + Arrays.toString(stack2));
	}
