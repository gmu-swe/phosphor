...
public class UninstrumentedCompatMV extends TaintAdapter {
	...
	public UninstrumentedCompatMV(int access, String className, String name, String desc,
	...
	public void visitTypeInsn(int opcode, String type) {
		if (opcode == Opcodes.CHECKCAST) {
			if (!analyzer.stack.isEmpty()
					&& "java/lang/Object".equals(analyzer.stack.get(analyzer.stack.size() - 1))
					&& type.startsWith("[")
					&& type.length()==2) {
	...
	public void visitInsn(int opcode) {
		...
		case Opcodes.AASTORE:
			...
			if(arType.equals("[Ljava/lang/Object;")
					&& ((elType.getSort() == Type.ARRAY
							&& elType.getElementType().getSort() != Type.OBJECT)
					...
					&& ((String)arType).contains(
						"[Ledu/columbia/cs/psl/phosphor/struct/multid/MultiDTainted")) {
					super.visitMethodInsn(Opcodes.INVOKESTATIC,
							Type.getInternalName(MultiDTaintedArray.class),
							"boxIfNecessary",
							"(Ljava/lang/Object;)Ljava/lang/Object;",
			...
			if (t.getDimensions() == 1
				...
				try {
					...
					super.visitTypeInsn(Opcodes.CHECKCAST,
							"[" + MultiDTaintedArray.getPrimitiveTypeForWrapper(
		...
		case Opcodes.ARRAYLENGTH:
			...
			if (onStack.getSort() == Type.OBJECT) {
				Type underlying = MultiDTaintedArray
								.getPrimitiveTypeForWrapper(onStack.getInternalName());
				if (underlying != null) {
					super.visitMethodInsn(Opcodes.INVOKESTATIC,
							Type.getInternalName(MultiDTaintedArray.class),
							"unbox1D",
	...
	public void visitMethodInsn(int opcode, String owner, String name,
		...
		if ((owner.equals("java/lang/System")
				...
				&& !desc.equals(
					"(Ljava/lang/Object;ILjava/lang/Object;IILjava/lang/DCompMarker;)V")) {
		...
		if (!isCalledOnArrayType
			...
			super.visitMethodInsn(opcode, owner, name, desc, itf);
		} else if (!Instrumenter.isIgnoredClass(owner)
				&& !owner.startsWith("[")
				&& !isCalledOnArrayType) {
			...
			if (desc.equals(TaintUtils.remapMethodDesc(desc))) {
				...
				for (int i = 0; i < args.length; i++) {
					...
					//		System.out.println(i + ", " + analyzer.stack.get(analyzer.stack.size() - argsSize)
				...
				for (int i = 0; i < args.length; i++) {
					...
					if (t.getSort() == Type.OBJECT) {
						...
						if (t.getDimensions() == 1
							...
							super.visitInsn(Opcodes.ARRAYLENGTH);
							if(Configuration.MULTI_TAINTING)
								super.visitTypeInsn(Opcodes.ANEWARRAY,
			...
			if (name.equals("<init>") && hasChangedDesc) {
				...
				if ((origReturnType.getSort() == Type.ARRAY
							&& origReturnType.getDimensions() == 1
							&& origReturnType.getElementType().getSort() != Type.OBJECT)
						|| (origReturnType.getSort() != Type.ARRAY
							&& origReturnType.getSort() != Type.OBJECT
							&& origReturnType.getSort() != Type.VOID)) {
					desc = desc.substring(0, desc.indexOf(')'))
			...
			} else if (origReturnType.getSort() != Type.ARRAY
					&& origReturnType.getSort() != Type.OBJECT
					&& origReturnType.getSort() != Type.VOID) {
				//unbox prim
				super.visitFieldInsn(GETFIELD,
