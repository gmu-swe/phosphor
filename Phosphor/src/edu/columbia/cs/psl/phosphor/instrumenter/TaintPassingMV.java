...
//							System.err.println("Setting the local type for " + shadowVar + analyzer.stack);
						if (opcode == ASTORE) {
							...
							analyzer.locals.set(shadowVar, Configuration.TAINT_TAG_ARRAY_INTERNAL_NAME);
						}
					}
				}
...
//				System.out.println("ASTORE " + var + " -> " + getTopOfStackType() + " but no shadow to be found");
	...
	public void visitFieldInsn(int opcode, String owner, String name, String desc) {
		...
		case Opcodes.PUTSTATIC:
			...
			if (shadowType == null && onStack.getSort() == Type.ARRAY && onStack.getElementType().getSort() != Type.OBJECT) {
				System.out.println("714ONSTack " + onStack);
	...
	public void retrieveTaintedArray(String type) {
		...
		super.visitFieldInsn(GETFIELD, Type.getInternalName(boxType), "taint", Configuration.TAINT_TAG_ARRAYDESC);
		//A TA
		super.visitInsn(SWAP);
...
//				System.out.println("zzzNEW " + wrapperType);
	...
	public void visitInvokeDynamicInsn(String name, String desc, Handle bsm, Object... bsmArgs) {
        ...
        boolean isIgnoredForTaints = Configuration.WITH_SELECTIVE_INST && Instrumenter.isIgnoredMethodFromOurAnalysis(owner, name, desc, false);
...
//      System.out.println("after: " + analyzer.stack);
	...
	public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean itfc) {
		...
		if (isCallToPrimitiveArrayClone) {
			...
			super.visitMethodInsn(opcode, Configuration.TAINT_TAG_ARRAY_INTERNAL_NAME, "clone", "()Ljava/lang/Object;",false);
		...
		if (owner.startsWith("edu/columbia/cs/psl/phosphor") && !name.equals("printConstraints") && !name.equals("hasNoDependencies") && !desc.equals("(I)V") && !owner.endsWith("Tainter")
				&& !owner.endsWith("ObjectMethodHelper") && !name.equals("getPHOSPHOR_TAG") && !name.equals("setPHOSPHOR_TAG")) {
			super.visitMethodInsn(opcode, owner, name, desc, itfc);
			return;
		}
		...
		if ((isIgnoredClass || isIgnoredForTaints || Instrumenter.isIgnoredMethod(owner, name, desc))  && !owner.startsWith("edu/columbia/cs/psl/phosphor/runtime")){
            ...
            super.visitMethodInsn(opcode, owner, name, desc, itfc);
				    return;
						...
						super.visitMethodInsn(INVOKEINTERFACE, t.getInternalName(), "toString$$PHOSPHORTAGGED", "(" + Configuration.TAINTED_RETURN_HOLDER_DESC + ")Ljava/lang/String;", true);
						return;
					}
					else
					{
						...
						else
						{
							FrameNode fn = getCurrentFrameNode();
							super.visitInsn(DUP);
							Label callOnObj = new Label();
							Label ok = new Label();
							Type t = Type.getType((Configuration.MULTI_TAINTING ? TaintedWithObjTag.class : TaintedWithIntTag.class));
							super.visitTypeInsn(INSTANCEOF, t.getInternalName());
							super.visitJumpInsn(IFEQ, callOnObj);
							super.visitTypeInsn(CHECKCAST, t.getInternalName());
							super.visitVarInsn(ALOAD, lvs.lvOfSingleWrapperArray);
							super.visitMethodInsn(INVOKEINTERFACE, t.getInternalName(), "toString"+TaintUtils.METHOD_SUFFIX, "("+Configuration.TAINTED_RETURN_HOLDER_DESC+")Ljava/lang/String;", true);
							FrameNode fn2 = getCurrentFrameNode();
							super.visitJumpInsn(GOTO, ok);
							super.visitLabel(callOnObj);
							acceptFn(fn);
				...
				} else if (TaintUtils.PREALLOC_RETURN_ARRAY && name.equals("equals") && desc.equals("(Ljava/lang/Object;)Z") && opcode == INVOKEVIRTUAL
						&& owner.equals("java/lang/Object")) {
					Type callee = getStackTypeAtOffset(1);
					boolean pop2 = false;
					if(TaintUtils.isPrimitiveArrayType(getTopOfStackType()))
					{
						callee = getStackTypeAtOffset(2);
						pop2 = true;
					}
					if(TaintUtils.isPrimitiveArrayType(callee))
					{
						if(pop2){
							super.visitInsn(SWAP);
							super.visitInsn(POP);
					...
					else
					{
						...
						super.visitVarInsn(ALOAD, lvs.lvOfSingleWrapperArray);
						Type container = TaintUtils.getContainerReturnType("Z");
						super.visitMethodInsn(INVOKEINTERFACE, t.getInternalName(), "equals$$PHOSPHORTAGGED", "(Ljava/lang/Object;"+Configuration.TAINTED_RETURN_HOLDER_DESC+")"+container.getDescriptor(), true);
						super.visitInsn(DUP);
						super.visitFieldInsn(GETFIELD, container.getInternalName(), "taint", Configuration.TAINT_TAG_DESC);
						super.visitInsn(SWAP);
						super.visitFieldInsn(GETFIELD, container.getInternalName(), "val", "Z");
				...
				} else if (TaintUtils.PREALLOC_RETURN_ARRAY && name.equals("hashCode")&& desc.equals("()I") && opcode == INVOKEVIRTUAL
						&& owner.equals("java/lang/Object")) {
					...
					else
					{
						...
						super.visitTypeInsn(CHECKCAST, t.getInternalName());
						super.visitVarInsn(ALOAD, lvs.lvOfSingleWrapperArray);
						super.visitMethodInsn(INVOKEINTERFACE, t.getInternalName(), "hashCode$$PHOSPHORTAGGED", "("+Configuration.TAINTED_RETURN_HOLDER_DESC+")"+Configuration.TAINTED_INT_DESC, true);
						super.visitInsn(DUP);
						super.visitFieldInsn(GETFIELD, Configuration.TAINTED_INT_INTERNAL_NAME, "taint", Configuration.TAINT_TAG_DESC);
						super.visitInsn(SWAP);
				...
				System.out.println(name+desc+"PRE XALOAD " + elType + ": " + analyzer.stack + "; " + analyzer.locals);
			{
				...
				if(Configuration.IMPLICIT_TRACKING)
				{
					...
					if (opcode == LALOAD || opcode == DALOAD) {
						...
						{
							super.visitInsn(POP);
							super.visitMethodInsn(Opcodes.INVOKESTATIC, Type.getInternalName(Taint.class), "copyTaint", "("+Configuration.TAINT_TAG_DESC+")"+Configuration.TAINT_TAG_DESC, false);
						}
						else
				...
				else
				{
					...
					super.visitMethodInsn(Opcodes.INVOKEVIRTUAL,  "edu/columbia/cs/psl/phosphor/runtime/LazyArrayIntTags", "get", "([" + elType + "II"  + retType.getDescriptor()+")"+retType.getDescriptor(), false);
					super.visitInsn(DUP);
...
//				System.out.println("33POST-AASTORE w/ mutlid array " + t + ": " + analyzer.stack);
		...
		case Opcodes.SASTORE:
			...
			if(isTaintlessArrayStore)
				super.visitMethodInsn(Opcodes.INVOKEVIRTUAL,  "edu/columbia/cs/psl/phosphor/runtime/LazyArrayIntTags", "set", "([" + elType + "II"  + elType+")V", false);
			else
				super.visitMethodInsn(Opcodes.INVOKEVIRTUAL,  "edu/columbia/cs/psl/phosphor/runtime/LazyArrayIntTags", "set", "([" + elType + "III"  + elType+")V", false);
			isTaintlessArrayStore = false;
