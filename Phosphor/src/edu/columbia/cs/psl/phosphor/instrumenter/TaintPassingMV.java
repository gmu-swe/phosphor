...
public class TaintPassingMV extends TaintAdapter implements Opcodes {
	...
	// Used to indicate that a call to push onto the ControlTaintTagStack should not be made for a particular branch
	...
	private final Label popAllLabel = new Label();
	...
	private final HashSet<MethodNode> wrapperMethodsToAdd;
	private boolean isExcludedFromControlTrack;
	final boolean isImplicitLightTracking;
	private final HashMap<Integer, Object> varTypes = new HashMap<>();
	private final HashSet<Integer> boxAtNextJump = new HashSet<>();
	private int branchStarting = DO_NOT_TRACK_BRANCH_STARTING;
	private final HashSet<Integer> forceCtrlAdd = new HashSet<>();
	private final List<Field> forceCtrlStoreFields = new LinkedList<>();
	private HashSet<Integer> varsNeverToForceBox = new HashSet<>();
	...
	/**
	 * Calls "push" on the ControlTaintTagStack, possibly requiring loading the exception data
	 */
	void callPushControlTaint(int idx) {
		super.push(idx);
	...
	void callPushControlTaintObj(int idx) {
		super.push(idx);
		super.push(sizeOfControlTaintArray);
		if(lvs.idxOfMasterExceptionLV >= 0) {
			super.visitVarInsn(ALOAD, lvs.idxOfMasterExceptionLV);
	...
	TaintPassingMV(MethodVisitor mv, int access, String className, String name, String desc, String signature, String[] exceptions, String originalDesc, NeverNullArgAnalyzerAdapter analyzer, MethodVisitor passThroughMV, HashSet<MethodNode> wrapperMethodsToAdd, boolean isImplicitLightTracking) {
		...
		this.lastArg = isStatic ? 0 : 1; // If non-static then arg[0] is this
		isExcludedFromControlTrack = false;
	...
	public void visitVarInsn(int opcode, int var) {
		...
		if(nextLoadisTracked && opcode == ALOAD) {
		...
		if(!nextLoadisTracked && opcode < 200) {
			if((isImplicitLightTracking || Configuration.IMPLICIT_TRACKING) && opcode == ASTORE) {
				super.visitInsn(DUP);
			...
			if(opcode == ASTORE && topCarriesTaint()) {
			...
			if(opcode == TaintUtils.NEVER_AUTOBOX) {
			...
			} else if(opcode == TaintUtils.ALWAYS_AUTOBOX && analyzer.locals.size() > var && analyzer.locals.get(var) instanceof String) {
				...
				if(opcode == ASTORE && TaintUtils.DEBUG_FRAMES) {
				...
				if(!finished) {
					if(var == 0 && !isStatic) {
						...
						if(var < lastArg && paramTypes[var] != null && TaintUtils.getShadowTaintType(paramTypes[var].getDescriptor()) != null) {
							...
							if(oldVarType != null) {
								...
								if((oldVarType == Opcodes.NULL || oldVarType instanceof String) && opcode != ASTORE && opcode != ALOAD) {
									...
									varTypes.put(var, opcode == ASTORE ? getTopOfStackObject() : "Lunidentified;");
								...
								switch (opcode) {
									...
									case ALOAD:
										...
										if(analyzer.locals.get(var) instanceof Label) {
											// This var is uninitialized obj. def not an array or anything we care about
											super.visitVarInsn(opcode, var);
											break;
										}
			...
			if(Instrumenter.isUninstrumentedField(owner, name)) {
				...
				switch (opcode) {
					...
					case GETSTATIC:
						...
						super.visitFieldInsn(opcode, owner, name, desc);
						super.visitMethodInsn(INVOKESTATIC, Type.getInternalName(MultiDTaintedArray.class), "boxIfNecessary", "(Ljava/lang/Object;)Ljava/lang/Object;", false);
						break;
					case PUTFIELD:
					case PUTSTATIC:
	 ...
	 *
	 * @param n the number of element on top of the inserted null in the final the stack
	 */
	...
	/**
	 * Removes the nth element down from the top (if n=0, the top of the stack is removed).
	 *
	 * @param n the number of elements on top of the element to be removed from the stack
	 */
		...
		if(bsmArgs != null) {
			...
			if(bsm.getName().equals("metafactory")) {
				...
				if(!Instrumenter.isIgnoredClass(implMethod.getOwner()) && !Instrumenter.isIgnoredMethod(implMethod.getOwner(), implMethod.getName(), implMethod.getDesc()) && !TaintUtils.remapMethodDescAndIncludeReturnHolder(implMethod.getDesc()).equals(implMethod.getDesc())) {
					...
					String remappedInstantiatedDesc = TaintUtils.remapMethodDescAndIncludeReturnHolderInit(instantiatedType.getDescriptor());
...
//					        newWrapperArgs.add(TaintUtils.getContainerReturnType(uninstSamMethodType.getReturnType()));
						...
						for (int j = 0; j < newWrapperArgs.size(); j++) {
							ga.visitVarInsn(newWrapperArgs.get(j).getOpcode(ILOAD), offset);
				...
				if(bsmArgs.length > 1 && bsmArgs[1] instanceof Handle) {
					...
					if(TaintUtils.isPrimitiveType(t.getReturnType())) {
						...
						if(_t.getReturnType().getSort() == Type.VOID) {
							...
							nd.append(TaintUtils.getContainerReturnType(t.getReturnType())).append(")V");
	...
	private static boolean isInternalTaintingMethod(String owner) {
	...
	public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean isInterface) {
		...
		boolean isCallToPrimitiveArrayClone = opcode == INVOKEVIRTUAL && desc.equals("()Ljava/lang/Object;")
				&& name.equals("clone") && getTopOfStackType().getSort() == Type.ARRAY
		...
		if(opcode == INVOKEVIRTUAL && Configuration.WITH_HEAVY_OBJ_EQUALS_HASHCODE && (name.equals("equals")
			...
			desc = name.equals("equals") ? "(Ljava/lang/Object;Ljava/lang/Object;)Z" : "(Ljava/lang/Object;)I";
		}
		...
		if((Instrumenter.isIgnoredClass(owner) || isIgnoredForTaints || Instrumenter.isIgnoredMethod(owner, name, desc))  && !isInternalTaintingMethod(owner)){
			...
			for (int i = 0; i < args.length; i++) {
				...
				if(args[args.length - i - 1].getSort() == Type.ARRAY && args[args.length - i - 1].getElementType().getSort() != Type.OBJECT
						&& args[args.length - i - 1].getDimensions() > 1) {
					if(!isIgnoredForTaints) {
						ensureUnBoxedAt(i, args[args.length-i-1]);
					}
				}
				else if(isPrimitiveType(args[args.length - i - 1]) || (args[args.length - i - 1].equals(Type.getType(Object.class))
...
//				// Z A B OR Z Z' A B?
	...
	@Override
	public void visitTableSwitchInsn(int min, int max, Label defaultCase, Label... labels) {
		if (isIgnoreAllInstrumenting) {
			super.visitTableSwitchInsn(min, max, defaultCase, labels);
		} else {
			...
			Configuration.taintTagFactory.tableSwitch(min, max, defaultCase, labels, mv, lvs, this);
		}
	}

	@Override
	public void visitLookupSwitchInsn(Label defaultCase, int[] keys, Label[] labels) {
		if(isIgnoreAllInstrumenting) {
			super.visitLookupSwitchInsn(defaultCase, keys, labels);
			return;
		}
		//Need to remove taint
		if((Configuration.IMPLICIT_TRACKING || isImplicitLightTracking) && branchStarting != DO_NOT_TRACK_BRANCH_STARTING) {
		...
		Configuration.taintTagFactory.lookupSwitch(defaultCase, keys, labels, mv, lvs, this);
	}
	...
	/**
	 * Has the specified method visitor generates an instruction to push the specified  value onto the stack.
	 *
	 * @param mv the method visitor that should generate the instruction
