				...
				protected void newControlFlowEdge(int insn, int successor, boolean isExceptionalEdge) {
					...
					if (implicitAnalysisblocks.containsKey(successor))
						...
						if (succesorBlock.insn.getType() == AbstractInsnNode.IINC_INSN) {
							...
							if ((Configuration.TRACK_METHODS_NOT_CALLED && !Configuration.taintTagFactory.isIgnoredClassFromControlFlowSummarization(min.owner) && !Instrumenter.isIgnoredClass(min.owner) && !Instrumenter.isIgnoredMethod(min.owner, min.name, min.desc)) && !min.owner.equals("java/lang/Enum")) {
								Frame fr = this.getFrames()[successor];
