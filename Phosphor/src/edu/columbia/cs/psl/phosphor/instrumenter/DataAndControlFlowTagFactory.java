...
public class DataAndControlFlowTagFactory implements TaintTagFactory, Opcodes {
	...
	@Override
	public void methodEntered(String owner, String name, String desc,
			MethodVisitor mv, LocalVariableManager lvs, TaintPassingMV ta) {}
	...
	@Override
	public void intOp(int opcode, int arg,
			MethodVisitor mv, LocalVariableManager lvs, TaintPassingMV adapter) {
		switch (opcode) {
	...
	public void stackOp(int opcode,
		...
		case Opcodes.ARRAYLENGTH:
			Type arrType = TaintAdapter.getTypeForStackType(
					adapter.analyzer.stack.get(adapter.analyzer.stack.size() - 1));
			{
	...
	@Override
	public void jumpOp(int opcode, int branchStarting, Label label,
			MethodVisitor mv, LocalVariableManager lvs, TaintPassingMV ta) {
	...
	public void typeOp(int opcode, String type,
		...
		case Opcodes.ANEWARRAY:
			if (Configuration.ARRAY_LENGTH_TRACKING
				...
				// We should just drop the taint for the size of the new array
				mv.visitInsn(SWAP); 
				mv.visitInsn(POP);
				Type t = Type.getType(type);
				if (t.getSort() == Type.ARRAY
		...
		case Opcodes.INSTANCEOF:
			...
			if (t.getSort() == Type.ARRAY
				...
				if (t.getDimensions() > 1) {
					...
					doIOR = true;
					// Maybe we have it boxed on the stack,
					// maybe we don't - how do we know? Who cares, just check both...
					mv.visitInsn(DUP);
					mv.visitTypeInsn(INSTANCEOF,
							Type.getInternalName(
								MultiDTaintedArray.getClassForComponentType(
									t.getElementType().getSort())));
					mv.visitInsn(SWAP);
				}
			}
		...
		case Opcodes.CHECKCAST:
			...
			if (t.getSort() == Type.ARRAY
					&& t.getElementType().getSort() != Type.OBJECT) {
				if (t.getDimensions() > 1) {
					// Hahaha you thought you could cast to
					// a primitive multi dimensional array!
					mv.visitTypeInsn(opcode,
							MultiDTaintedArray.getTypeForType(
					...
					if (o instanceof String) {
						...
						if (zz.getSort() == Type.ARRAY
								&& zz.getElementType().getSort() != Type.OBJECT) {
							mv.visitTypeInsn(opcode, type);
							break;
						}
					}
	...
	@Override
	public void iincOp(int var, int increment,
			MethodVisitor mv, LocalVariableManager lvs, TaintPassingMV ta) {

		if (Configuration.IMPLICIT_TRACKING
				&& !Configuration.WITHOUT_PROPOGATION) {
			if (ta.isIgnoreAllInstrumenting
					|| ta.isRawInsns) {
			...
			if (var < ta.lastArg
					&& TaintUtils.getShadowTaintType(
						ta.paramTypes[var].getDescriptor()) != null) {
